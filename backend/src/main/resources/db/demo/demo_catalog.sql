-- DU LIEU MAU — CHI CHAY O PROFILE `local` (may lap trinh vien).
-- DemoCatalogSeeder nap file nay khi bang `items` con trong. Production khong bao gio
-- chay file nay: chu shop tu nhap dan that qua trang quan tri.
--
-- Anh la placeholder (route /media cua frontend ve o ke soc).
...
INSERT INTO inspection_checks (inspection_id, code, label, group_name, passed, sort_order)
SELECT ins.id, c.code, c.label,
       CASE
         WHEN c.code IN ('NECK_RELIEF','TRUSS_ROD','NECK_JOINT','NECK_TWIST','ACTION_1','ACTION_12',
                         'NUT_SLOT','FRET_LEVEL','FRET_ENDS','FRET_WEAR','FRETBOARD') THEN 'Cần & mặt phím'
         WHEN c.code IN ('PICKUP_HEIGHT','PICKUP_OUTPUT','SWITCH','POT_VOLUME','POT_TONE','JACK',
                         'SOLDER','SHIELDING','GROUND_HUM') THEN 'Điện & tiếng'
         WHEN c.code IN ('TUNER_HOLD','TUNER_SMOOTH','STRING_TREE','BRIDGE_SEAT','SADDLE','TREMOLO',
                         'SPRING_CLAW','HARDWARE') THEN 'Phần cứng'
